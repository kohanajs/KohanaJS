...
describe('Controller Mixin View Test', () => {
  ...
  test('do not render non text content', async()=>{
    class C extends Controller {
      static mixins = [ControllerMixinView];
    ...
    const c = new C({});
    c.headers['Content-Type'] = 'application/pdf';
    await c.execute('test');
    expect(typeof c.body).toBe('object');
  })

  test('url with ?cp', async () =>{

  })

  test('url without ?cp', async()=>{

  })

  test('todo: code coverage branch: when state defined', async() =>{

  })

  test('todo: code coverage branch: when template is object', async() =>{
